...
export function AxiosInterceptors({ children }) {
    ...
    useEffect(() => {
        ...
        const respInterceptor = authResponseInterceptor(
            isRefreshing,
...
export function authResponseInterceptor(isRefreshing, refreshAndRetryQueue, authContext) {
    const { refreshToken, logout, updateAccessToken } = authContext;

    return PRIVATE_API.interceptors.response.use(
        (response) => response,
